window.onload = function()
{
    ...
    */
    ...
    var decimalFraccionario = function()
    {
        ...
        var exp = Math.pow(10,String(datosDecimal.decimal).length);        
        var res = ((datosDecimal.entero * exp) + datosDecimal.decimal);
        var tot  = res + "/" + exp;
        var resultado = simplificaFraccionario(tot);
        
        console.log(datosDecimal);
        //Imprime la respuesta en la vista (HTML)...
        nom_div("resFraccionario").innerHTML = resultado;
    };
    ...
    */
    ...
    { 
            ...
            var res = 0;
            var indice = 0;
            for(var i = numeroInputs; i>=1; i--)
            {
                var ind = "valor_" + i;
                var numero =  Number(nom_div(ind).value);
                //indice = numeroInputs - cont;
                cont++;
                if(numero == 1)
                {                  
                    res = res + Math.pow(2, cont);
                }
            }   
        /*
    ...
    */
    ...
    nom_div("maximo").addEventListener('change', function(event)
    {
        ...
        nom_div("resSumatoria").innerHTML = mod;
    });
